...
from assignment_1_code.models.vit import vitModel
...
def train(args):
    ...
    net = vitModel(image_size=32, patch_size=4, num_classes=10,
                   emb_size=128, depth=4, heads=4)
    net.to(device)

    # Optimizer, loss, and learning rate scheduler
    optimizer = torch.optim.AdamW(
        net.parameters(), lr=0.001, amsgrad=True, weight_decay=1e-4
    )
    ...
    # Create trainer (ViT model goes directly, no wrapper needed)
...
if __name__ == "__main__":
    args = argparse.ArgumentParser(description="Train Your ViT")
